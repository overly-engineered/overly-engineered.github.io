...
var radius = width > height ? height / 2 - 50 : width - 50;
var startAngle = 0;
...
function create() {
  for (var i = 0; i < circleAmount; i++) {
    var cDirection = Math.random() > 0.5 ? true : false;
    circles.push({
      x: centerX,
      y: centerY,
...
function drawCircle(elem, i, arr) {
  ...
  if (i + 1 === arr.length && !interaction) {
    interaction = false;
    window.globalAnim = window.requestAnimationFrame(draw);
  }
}
...
function handleMouse(e) {
  interaction = true;
  cancelAnimationFrame(window.globalAnim);
